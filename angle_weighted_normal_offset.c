...
int main() {
    ...
    for (int i = 0; i < num_vertices; i++) {
        ...
        // weigh normal vector by angle relative to centroid
        float angle = compute_angle(v1, v2);
        angle_weighted_norm[0] += angle * normal[0];
        angle_weighted_norm[1] += angle * normal[1];
        angle_weighted_norm[2] += angle * normal[2];
    }

    printf("Angle Weighted Normal: (%f, %f, %f)\n", angle_weighted_norm[0], angle_weighted_norm[1], angle_weighted_norm[2]);
    // I believe this will return the point where new normal vector points to
    // then base of normal vector is the polygon centroid?
